...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as alex:
...
def pyro_session():
    ...
    try:
        with Client(name="alex", api_id=API_ID, api_hash=API_HASH, in_memory=True) as pyro:
            ss = pyro.export_session_string()
            pyro.send_message(
                "me",
                f"`{ss}`\n\nAbove is your Pyrogram Session String. **DO NOT SHARE it like your ass.**",
            )
...
def main():
    ...
    try:
        type_of_ss = int(
            input(
                "\nYou can generate both telethon as well as pyrogram sessions.\n\nWhich session do you want to generate?\n1. Telethon Session.\n2. Pyrogram Session.\n\nEnter your choice:  "
            )
        )
